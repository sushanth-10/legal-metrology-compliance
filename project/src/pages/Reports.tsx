      ...
      {reportsLoading && reports.length === 0 ? (
        ...
        <div className="card p-8 text-center">
          ...
          <p className="text-sm text-ink-500 mt-2 max-w-md mx-auto">
            Generate an official PDF report from an officer-accessible scan result to see it here.
          </p>
        </div>
      ) : (
              ...
              <tbody className="divide-y divide-ink-100">
                {reports.map((report) => (
                  <tr key={report.id} className="hover:bg-ink-50/60">
                    ...
                    <td className="table-td text-ink-500"><div>{new Date(report.scanDate ?? report.generatedAt).toLocaleString()}</div><div className="text-xs mt-1">Generated {new Date(report.generatedAt).toLocaleString()}</div></td>
          ...
          <div className="w-full max-w-3xl max-h-[90vh] overflow-y-auto rounded-2xl border border-ink-200 bg-white p-5 shadow-2xl">
            ...
            <div className="mt-5 rounded-xl bg-ink-50 border border-ink-200 p-4 text-sm text-ink-700">
              <div className="grid sm:grid-cols-2 gap-3">
                ...
                <div><span className="font-semibold text-ink-800">Generated by:</span> {previewReport.officerName ?? 'Unavailable'}</div>
              </div>
            ...
            <div className="mt-5 flex flex-col sm:flex-row gap-3 justify-end">
              <button onClick={() => void download(previewReport)} className="btn-primary py-2.5">
