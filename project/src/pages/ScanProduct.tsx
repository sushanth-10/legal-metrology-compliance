...
function Processing({ mode, results, back }: { mode: Mode; results: () => void; back: () => void }) {
  ...
  return <div className="max-w-5xl mx-auto"><PageHeader title={mode === 'advanced' ? 'Advanced Scan in progress' : 'Generating compliance report'} subtitle="Gemini extraction and Python compliance checks are running securely." /><div className="grid lg:grid-cols-[1.15fr_.85fr] gap-6"><section className="card p-5 sm:p-7"><div className="flex items-center gap-3 pb-5 border-b border-ink-100"><div className="w-11 h-11 rounded-xl bg-brand-50 text-brand-600 grid place-items-center"><ScanLine className="w-5 h-5" /></div><div><h2 className="font-semibold text-ink-900">Processing timeline</h2><p className="text-sm text-ink-500">Your uploaded images are being analyzed by the secure backend.</p></div></div><ol className="mt-6">{steps.map((label, i) => <li key={label} className="flex gap-3 min-h-14"><div className="flex flex-col items-center">{i < 2 ? <CheckCircle2 className="w-5 h-5 text-success-500" /> : i === 2 ? <Loader2 className="w-5 h-5 text-brand-600 animate-spin" /> : <Circle className="w-5 h-5 text-ink-300" />}{i < steps.length - 1 && <div className={'w-px flex-1 my-1 ' + (i < 2 ? 'bg-success-200' : 'bg-ink-200')} />}</div><div className="pb-5"><p className={'text-sm font-medium ' + (i < 2 ? 'text-ink-800' : i === 2 ? 'text-brand-700' : 'text-ink-400')}>{label}</p>{i === 2 && <p className="text-xs text-ink-500 mt-0.5">Waiting for extractor events</p>}</div></li>)}</ol></section><aside className="space-y-4"><div className="card p-5"><span className="badge bg-brand-50 text-brand-700"><Loader2 className="w-3.5 h-3.5 animate-spin" />{state}</span><h2 className="font-semibold text-ink-900 mt-4">Live data flow</h2><p className="text-sm text-ink-600 mt-2 leading-6">Images → Gemini extraction → structured observations → Python compliance_engine → results.</p><p className="text-sm text-ink-600 mt-2 leading-6">Gemini extracts only; Python rules determine compliance.</p></div><InfoNote>The result screen will preserve the actual extracted values, explanations, and evidence returned by the backend.</InfoNote><button onClick={results} className="btn-primary w-full py-3"><FileText className="w-4 h-4" />View result</button><button onClick={back} className="btn-secondary w-full py-3"><ArrowLeft className="w-4 h-4" />Back to images</button></aside></div></div>;
}
...
function PersistedReportActions({ result }: { result: ScanApiResponse | null }) {
  ...
  if (role !== 'officer' || !result?.scan) return null;
