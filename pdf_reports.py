...
def _certificate_qr(value: str, size: float = 28 * mm) -> Drawing:
    qr = QrCodeWidget(value)
    x1, y1, x2, y2 = qr.getBounds()
    scale_x = size / max(1, x2 - x1)
    scale_y = size / max(1, y2 - y1)
    drawing = Drawing(size, size, transform=[scale_x, 0, 0, scale_y, -x1 * scale_x, -y1 * scale_y])
    drawing.add(qr)
    return drawing
...
def create_certificate_pdf(certificate: dict[str, Any], output_path: Path) -> None:
    ...
    document = CertificateDocument(
        str(output_path), pagesize=A4, leftMargin=margin, rightMargin=margin,
        topMargin=14 * mm, bottomMargin=15 * mm,
        title="NIRIKSHA Compliance Assessment Certificate", author="NIRIKSHA",
    )
    frame = Frame(document.leftMargin, document.bottomMargin, content_width, page_height - document.topMargin - document.bottomMargin, id="certificate")

    def draw_certificate_page(canvas: Any, doc: Any) -> None:
        canvas.saveState()
    ...
    if packer != "Not available" and packer != manufacturer:
        manufacturer = f"{manufacturer}; Packer: {packer}"
    product_name = certificate.get("product_name") or "Product name unavailable"
    brand = _field_value(fields, "brand")
